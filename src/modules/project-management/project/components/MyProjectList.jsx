...
const statusConfig = {
  Planned: {
    ...
    icon: <FiClock className="w-4 h-4 text-amber-600" />,
  },
  "In Progress": {
    badge: "bg-blue-50 text-blue-700 border-blue-200",
    icon: <FiAlertCircle className="w-4 h-4 text-blue-600" />,
  },
  Completed: {
    badge: "bg-emerald-50 text-emerald-700 border-emerald-200",
    icon: <FiCheckCircle className="h-4 h-4 text-emerald-600" />,
  },
};
...
export default function MyProjectsList({ employeeId }) {
  ...
  const { employeeProjects = [], status = {} } = useSelector(
    (state) => state.project || {}
  );
  ...
  useEffect(() => {
    if (employeeId) dispatch(fetchProjectsByEmployeeId(employeeId));
  }, [dispatch, employeeId]);
  ...
  const projectStats = Array.isArray(employeeProjects)
    ? {
        ...
        inprogress: employeeProjects.filter((p) => p.status === "In Progress")
          .length,
        completed: employeeProjects.filter((p) => p.status === "Completed")
          .length,
      }
    ...
    >
      ...
      <CardContent className="space-y-5 flex-1">
        ...
        <div className="space-y-1">
          <h3 className="text-[15px] font-semibold leading-snug line-clamp-2 text-primary">
        ...
        <div className="space-y-4 text-sm">
          ...
          {/* Expected — hide if completed */}
          {project.expectedEndDate && !project.endDate && (
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium">
                  {formatDateUTC(project.expectedEndDate)}
                </p>
